...
class _ResidentBookingsTabState extends State<ResidentBookingsTab> {
      ...
      if (bookingsResponse['success'] == true) {
        ...
        if (mounted) {
          setState(() {
            ...
            _filteredBookings = List.from(userBookings);
            _isLoading = false;
          });
        }
      } else {
  ...
  void _updateFilters() {
    setState(() {
      _filteredBookings = _applyFilters(
        _bookings,
  ...
  Widget _buildBookingCard(Map<String, dynamic> booking) {
    ...
    print('🔍 _buildBookingCard: $facilityName');
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            _buildDetailRow(icon: Icons.calendar_today, label: 'Date', value: booking['booking_date'] ?? booking['date'] ?? 'Not set'),
            const SizedBox(height: 8),
            _buildDetailRow(icon: Icons.access_time, label: 'Time Slot', value: booking['start_time'] ?? booking['timeslot'] ?? booking['time_slot'] ?? 'Not set'),
            const SizedBox(height: 8),
            _buildDetailRow(icon: Icons.attach_money, label: 'Total Amount', value: '₱${booking['total_amount'] ?? booking['totalAmount'] ?? '0'}'),
            if (booking['downpayment'] != null) ...[
              const SizedBox(height: 8),
              _buildDetailRow(icon: Icons.payment, label: 'Downpayment', value: '₱${booking['downpayment']}'),
            ],
            ...
            Builder(
              builder: (context) {
                ...
                print('🔍 Booking created_at field: $createdAt');
                return Text(
  ...
  Widget _buildDetailRow({required IconData icon, required String label, required String value}) {
    return Row(
      children: [
        Icon(icon, size: 16, color: widget.isDarkMode ? Colors.grey.shade400 : Colors.grey[600]),
        const SizedBox(width: 8),
        Text(
          '$label:',
          style: TextStyle(
      ...
      child: Column(
        ...
        children: [
          ...
          _buildFilterDropdown(
            ...
            (value) {
              if (value != null) {
                setState(() => _selectedFacilityFilter = value);
                _updateFilters();
              }
            },
          ),
            ...
            children: [
              Expanded(
                child: _buildFilterDropdown(
                  ...
                  (value) {
                    if (value != null) {
                      setState(() => _selectedSubmittedDateSort = value);
                      _updateFilters();
                    }
                  },
                ),
              ...
              Expanded(
                child: _buildFilterDropdown(
                  ...
                  (value) {
                    if (value != null) {
                      setState(() => _selectedBookingDateSort = value);
                      _updateFilters();
                    }
                  },
                ),
                ...
                child: TextField(
                  ...
                  decoration: InputDecoration(
                    ...
                    focusedBorder: OutlineInputBorder(
                      borderSide: BorderSide(color: widget.isDarkMode ? Colors.blue.shade300 : Colors.blue),
                    ),
                  ),
                  onChanged: (value) {
  ...
  // ── KEY FIX: dropdownColor + icon color + underline color ─────────────────
    ...
    return Column(
      ...
      children: [
        ...
        DropdownButton<String>(
          ...
          isExpanded: true,
          isDense: true,
          dropdownColor: widget.isDarkMode ? Colors.grey.shade800 : Colors.white, // ← fixes popup background
          style: TextStyle(fontSize: 12, color: widget.isDarkMode ? Colors.white : Colors.black),
          underline: Container(
            height: 1,
